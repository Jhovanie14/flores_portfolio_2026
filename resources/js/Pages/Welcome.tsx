...
const STATIC_STACK: { category: string; skills: string[] }[] = [
    ...
    { category: "Backend",        skills: ["Laravel", "Node.js", "MySQL", "REST API"] },
    { category: "Design & Tools", skills: ["Figma", "Git", "Docker", "Inertia.js"] },
];
        ...
        <div className="overflow-hidden">
            ...
            >
                {repeated.map((item, i) => (
                    <div
                        key={i}
                        className="flex-shrink-0 flex flex-col items-center justify-center gap-2.5 w-24 h-24 border border-white/5 bg-white/[0.02] hover:border-[#c8b97a]/30 hover:bg-white/[0.04] transition-all duration-300 group"
                    >
                        {item.image ? (
                            <img
                                src={item.image}
...
export default function Home({ skills, stacks }: Props) {
    ...
    const skillGroups = skills.length > 0
        ? (() => {
            const ordered = CATEGORY_ORDER.reduce<{ category: string; skills: string[] }[]>((acc, cat) => {
                const group = skills.filter((s) => s.category === cat).map((s) => s.name);
                if (group.length) acc.push({ category: cat, skills: group });
                return acc;
            }, []);
            ...
            extra.forEach((cat) =>
                ordered.push({ category: cat, skills: skills.filter((s) => s.category === cat).map((s) => s.name) }),
            );
            return ordered;
        })()
        : STATIC_STACK;
    ...
    return (
        <Guest fullScreen>
            ...
            <section className="relative min-h-screen bg-[#0e0e0e] flex flex-col justify-end px-8 md:px-14 pb-16 overflow-hidden">
                ...
                <div className={`flex items-center gap-4 font-dm-sans text-[11px] font-light text-[#c8b97a] tracking-[0.25em] uppercase mb-5 ${revealed("delay-300")}`}>
                ...
                >
                    Jhovanie
                    <br />
                    <span style={{ color: "transparent", WebkitTextStroke: "1.5px #e8e2d4" }}>
                        Flores
                    </span>
                </h1>

                <div className={`flex flex-col md:flex-row md:justify-between md:items-end gap-8 mt-12 ${revealed("delay-[650ms]")}`}>
                ...
                <div className={`absolute bottom-10 left-8 md:left-14 flex items-center gap-3 font-dm-sans text-[11px] text-[#333] tracking-[0.12em] uppercase ${revealed("delay-[900ms]")}`}>
                    <span className="block w-10 h-px bg-[#333]" />
                    Scroll down
                </div>

                <div className={`absolute bottom-10 right-8 md:right-14 flex items-center gap-2 font-dm-sans text-[11px] text-[#444] tracking-[0.1em] uppercase ${revealed("delay-[1000ms]")}`}>
            ...
            {/* ── Stack tags (fallback when no images, uses skills) ── */}
            {/* {stacks.length === 0 && (
